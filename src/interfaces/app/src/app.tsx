...
import { removeStorageItem, setStorageItem } from "./core/storage/local-storage";
import { GatewayStartupService } from "./core/gateway/gateway-startup-service";
...
export default function App() {
  ...
  useEffect(() => {
    if (!window.electronAPI) {
      console.warn("[App] electronAPI not available, skipping gateway start");
      return;
    }
    ...
    const initializeGateway = async () => {
      try {
        const result = await service.autoStartGateway();
        if (result.success) {
          console.log("[App] Gateway started successfully");
        } else {
        ...
        console.error("[App] Gateway startup error:", errorMessage);
      }
    };

    // Start gateway even during onboarding - onboarding needs it to test connection
    initializeGateway();
  }, [showOnboarding]);
